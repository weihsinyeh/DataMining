...
def will_leave(attribute):
    probOfLeave = 0
    # 年齡 : >=65             離職率上升20%
    if attribute['age'] >= 65: probOfLeave += 0.2
    if attribute['age'] <= 25: probOfLeave += 0.1
    # 每天平均上班時間 : >= 14 離職率上升20%
    if attribute['working_time(hour)'] >= 14: probOfLeave += 0.2
    # 工作滿意度 : <= 2       離職機率上升20%
    if attribute['working_satisfaction'] <= 2: probOfLeave += 0.2
    # 薪資水平 : <= 25000     離職機率上升20%
    if attribute['salary'] <= 25000: probOfLeave += 0.05
    # 個人生活狀況 : 有小孩   離職機率上升20%
    if attribute['personal_life'] == 1: probOfLeave += 0.2
    # 參加公司社團的頻率 : 3 與 4 離職機率降低 5 %
    if attribute['frequency_of_attend_club'] in [3, 4]: probOfLeave -= 0.05
    # 距離上次加薪幾個月 : >= 18 離職率上升5% <=6 離職率下降20%
    if attribute['Bonus_month'] >= 18: probOfLeave += 0.05
    if attribute['Bonus_month'] <= 6: probOfLeave -= 0.2
    # 員工參與度(5: 會接下外務, 4: 出國會帶點心, 3: 會參加公司活動, 2: 會跟同事一起吃飯,1: 會跟同事一起加班) :離職率分別降低(20%, 10%, 5%, 5%, 5%)
    if attribute['working_participation'] == 5: probOfLeave -= 0.2
    if attribute['working_participation'] == 4: probOfLeave -= 0.1
    if attribute['working_participation'] == 3: probOfLeave -= 0.05
    if attribute['working_participation'] == 2: probOfLeave -= 0.05
    if attribute['working_participation'] == 1: probOfLeave -= 0.05
    # 上級主管關係 : <= 3 離職率上升10%
    if attribute['relationship'] <= 3: probOfLeave += 0.1
    # 出社會後跳槽平均頻率（0 : 一年內, 1 : 兩年內 2: 五年內, 3: 沒換過公司） 
    if attribute['change_job'] == 0: probOfLeave += 0.1
    # 公司職務 （1:助理,2:職員,3:組長,4:經理,5:處長,6:總裁,7董事長）:5 6 7 離職率為0
    if attribute['position'] in [5, 6, 7]: probOfLeave = 0
    # 健康度 : >=2 離職率上升5 %
    if attribute['health'] >= 2: probOfLeave += 0.05
    # 離職 : True , 不離職 : False
...
def generateDataSet(number):
  ...
  with open('output_with_fakse_data.csv', 'w', newline='') as csvfile:
...
def readDataset(filename):
  Dataset = []
  with open('output_with_fakse_data.csv', newline='') as csvfile:
